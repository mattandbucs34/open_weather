let defaultCity = encodeURI('Houston, Texas');
let defaultCoords = { latitude: 0, longitude: 0 };
let forecast;
let multiCity = ['New York, New York', 'Chicago, Illinois', 'Houston, Texas', 'Denver, Colorado', 'Los Angeles, California', 'Washington D.C., District of Columbia'];
...
const getLocalWeather = async (latitude, longitude) => {
    ...
    if (latitude, longitude) {
        ...
        const coords = await geoCodeCity(defaultCity);
        localWeatherData = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${coords.lat}&lon=${coords.lon}&appid=${open_weather_api}&units=imperial`);
    }
...
const fetchCity = async (city) => {
    const cityData = await fetch(`https://api.openweathermap.org/data/3.0/onecall?q=${city}&appid=${open_weather_api}&units=imperial`);
    return await cityData.json();
};

const fetchOneCall = async (lat, long) => {
    const oneCallData = await fetch(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${long}&appid=${open_weather_api}&units=imperial`);
